...
const AppointmentBooking: React.FC<AppointmentBookingProps> = ({ onBack, onNavigate, preSelectedPatientId }) => {
  ...
  const [patient, setPatient] = useState<any>(preSelectedPatientId ? { name: 'Aarav Kumar', id: '#DAI-8291', age: '7 years', avatar: 'Rivera' } : null);
          ...
          <div className="bg-slate-50 rounded-[2rem] border border-slate-200 p-8 shadow-sm mb-10 text-left">
             ...
             <div className="grid grid-cols-2 gap-6">
                <div>
                   ...
                   <p className="text-base font-bold text-slate-900">Thu, Oct {selectedDate}, 2024</p>
                 ...
                 <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4">
                    ...
                    <div>
                       ...
                       <div className="flex gap-4 overflow-x-auto pb-2">
                          {['Aarav', 'Priya', 'Arjun'].map((p, i) => (
                            <button 
                              key={i}
                              onClick={() => setPatient({ name: `${p} Kumar`, id: `#DAI-${8000+i}`, age: `${6+i} years`, avatar: p })}
                            ...
                            >
                               ...
                               <div className="text-left">
                                  ...
                                  <p className="text-[9px] font-bold text-slate-400">ID: #{8000+i}</p>
                               </div>
                            </button>
                          ))}
                       </div>
            ...
            <section className="space-y-4">
               ...
               <div className="bg-white rounded-[2rem] border border-slate-200 p-6 shadow-sm">
                  ...
                  <div className="flex items-center justify-between mb-6">
                     ...
                     <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">October 2024</h4>
                     <button className="p-2 hover:bg-slate-50 rounded-xl"><ChevronRight size={20} className="text-slate-400" /></button>
                  </div>
                  <div className="grid grid-cols-7 gap-2 mb-8">
                     {['S','M','T','W','T','F','S'].map(d => <div key={d} className="text-center text-[10px] font-bold text-slate-300 mb-2">{d}</div>)}
                     {[...Array(31)].map((_, i) => {
                        const day = i + 1;
                        ...
                        return (
                           <button 
                             ...
                             className={`h-10 rounded-xl text-xs font-bold flex items-center justify-center relative transition-all ${
                           ...
                           >
                              {day}
                              {[22,24,25,28].includes(day) && !isSelected && <div className="absolute bottom-1 w-1 h-1 rounded-full bg-green-500" />}
                           </button>
                        )
                     })}
                  </div>
                     ...
                     <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                        {timeSlots.map((slot, i) => (
                           <button 
                             ...
                             className={`py-3 px-2 rounded-xl text-[10px] font-black uppercase tracking-widest border-2 transition-all relative ${
                  ...
                  {[
                    ...
                    <button 
                      ...
                      className={`h-20 rounded-2xl border-2 flex flex-col items-center justify-center gap-2 transition-all ${
                        format === f.id ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-100 bg-white text-slate-400 hover:border-blue-200'
                      }`}
                    >
                       {f.icon}
                        ...
                        <div className="flex items-center gap-3">
                           ...
                           <div>
                              ...
                              <p className="text-xs font-black text-slate-800">
                                 {selectedDate ? `Oct ${selectedDate}, 2024` : '-'} 
