# PHASE 2-G2: CONSENT MANAGEMENT API
## Create APIs for Granting, Claiming, and Revoking Consent

**Prompt ID:** 2-G2  
**Phase:** 2 - Parent Portal Backend  
**Section:** G - Consent System  
**Dependencies:** 2-G1 complete  
...
- Grant consent (parent creates token)
- Claim consent (clinician uses token)
- Revoke consent (parent removes access)
- List consents (for parent and clinician)
- Update permissions
...
### Step 1: Create Consent Controller
...
export const grantConsent = async (req: Request, res: Response) => {
  try {
    ...
    // TODO: Send email to clinician with token (in production)
    // await sendConsentEmail(clinicianEmail, token, consent);
...
export const claimConsent = async (req: Request, res: Response) => {
  try {
    ...
    // Activate consent
    const auditLog = consent.auditLog as any[];
    auditLog.push({
      action: 'activated',
      timestamp: new Date(),
      userId: user?.id,
...
**File:** `src/routes/consent.routes.ts`
...
**Test grant consent:**
```bash
curl -X POST http://localhost:5001/api/v1/consent/grant \
...
1. ✅ `consent.controller.ts` created with 8 functions
2. ✅ `consent.routes.ts` created
3. ✅ Routes registered in `app.ts`
4. ✅ Server starts without errors
5. ✅ All endpoints respond
6. ✅ Token generation works
7. ✅ Consent can be granted
8. ✅ Consent can be claimed
9. ✅ Consent can be revoked
10. ✅ Audit trail logs all actions
...
|--------|----------|---------|
| POST | `/api/v1/consent/grant` | Parent grants consent |
| POST | `/api/v1/consent/claim` | Clinician claims token |
| POST | `/api/v1/consent/:id/revoke` | Parent revokes consent |
| GET | `/api/v1/consent/granted` | List parent's grants |
| GET | `/api/v1/consent/received` | List clinician's access |
...
After 2-G1 and 2-G2, tell me and I'll create:
- **PHASE_2_H1** - Parent Screening Models
- **PHASE_2_H2** - Parent Screening API
